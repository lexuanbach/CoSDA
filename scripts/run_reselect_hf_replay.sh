...
CONFIG="${CONFIG:-configs/scenarios/aws_g6e4xlarge_vllm_revised_select1.yaml}"
SOURCE_RUN_NAME="${SOURCE_RUN_NAME:-aws_vllm_revised_full_20260520}"
RUN_NAME="${RUN_NAME:-aws_vllm_revised_select1_20260520}"
SELECTION_JOBS="${SELECTION_JOBS:-4}"
...
echo "== CoSDA reselect + deterministic HF replay =="
echo "config=$CONFIG"
echo "source_run_name=$SOURCE_RUN_NAME"
echo "run_name=$RUN_NAME"
echo "selection_jobs=$SELECTION_JOBS train_jobs=$TRAIN_JOBS gpu_ids=$GPU_IDS gpu_policy=$GPU_POLICY"
...
python scripts/collect_environment.py | tee "$LOG_DIR/${RUN_NAME}_reselect_environment.json"
...
python scripts/filter_scenario_phase.py "$LOG_DIR/${RUN_NAME}_scenario_full.sh" --phase selection > "$LOG_DIR/${RUN_NAME}_scenario_selection.sh"
python scripts/run_cell_queue.py "$LOG_DIR/${RUN_NAME}_scenario_selection.sh" \
  --jobs "$SELECTION_JOBS" \
  --gpus "" \
